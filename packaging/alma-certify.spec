...
# MIT for this suite and everything vendored with it, except typing-extensions, which is
# PSF-2.0. See the bundled() list below and bundled/ for the notices.
...
# old dedicated tui package
Provides:       %{name}-tui = %{version}-%{release}
Provides:       almalinux-certify-tui = %{version}-%{release}
Obsoletes:      %{name}-tui < %{version}-%{release}
Obsoletes:      almalinux-certify-tui < %{version}-%{release}
...
It includes the full-screen guided interface, `alma-certify tui`: a mouse-driven
front-end that collects the run choices, shows the command they add up to, and
either hosts the run or hands it off to the same commands as the CLI. Textual
and its dependencies are bundled because they are not packaged on every
supported EPEL release. This was a separate alma-certify-tui subpackage; it is
part of this one now, so the guided interface is there the first time somebody
looks for it.
...
# the guided interface and its vendored stack, with our local pygments shim
%{alma_certify_python} -I -c 'import sys; \
    sys.path.insert(0, "%{buildroot}%{alma_certify_home}"); \
    ...
    print("guided interface import check OK")'
...
%changelog
* Mon Sep 21 2026 Jonathan Wright <jonathan@almalinux.org> - 1.0.0-1
