# alma-certify result schema (v1.2)
...
|---|---|---|
| `schema_version` | string | `"1.2"` (1.0 lacked `baseboard` and `system.kind`; `system.model_number`, `chassis`, and `bmc` were added later within 1.1 and are absent from older reports; 1.2 adds `summary.fields`) |
...
`inventory.summary.fields` is that same output *parsed*: every structure
`dmidecode` printed and every key `lspci` printed, as JSON, so a reader can
read a field without shipping a parser or asking for a new release of this
suite. The summary keys below are a schema and a contract; `fields` is not
one, and nothing here promises which keys a given machine reports.
...
- `fields`: `{dmi: {<structure>: [{handle, dmi_type, name, props}]}, pci: [...]}`
  - `dmi` is keyed by what the structure describes (`processor`, `system`,
    `memory_device`, `cache`, ...), falling back to `type_<n>` for a structure
    with no name here, so nothing is dropped for want of a translation. `props`
    is dmidecode's own keys and values, untouched.
  - `pci` is every key `lspci -vmmnnk` printed per device, verbatim.
    `pci_devices` above keeps six of them in a fixed shape because
    categorization is keyed on those; this keeps the rest.
  - Identity-bearing: serials and asset tags appear on most DMI structures, not
    only the three the summary promotes. `--redact` drops them here too.
