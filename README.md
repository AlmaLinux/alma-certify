...
`alma-certify` package with its interface library bundled, so it is there the first time you look
for it, and **it only appears when there is a terminal on both stdin and stdout**. In a script, a CI job, or a kickstart `%post`, a bare `alma-certify` prints
