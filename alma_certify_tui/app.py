...
# Aliased: ``textual.widgets.selection_list`` exports a wholly unrelated ``Selection`` that this
# module already imports below, and one of the two silently shadowing the other is the sort of
# bug that only shows up when somebody drags the mouse.
from textual.selection import Selection as TextSelection
...
from alma_certify.runhooks import RunHooks, run_log_path, write_log_line
...
def unavailable_reason() -> Optional[str]:
    ...
    Reaching this means the package imported, so Textual is present; what is left to rule out is
...
class _AuthScreen(ModalScreen):
    ...
    There is deliberately no copy-the-code key. It was ``c``, writing OSC 52, which is
    fire-and-forget: the terminal never acknowledges it, so the dialog claimed "code copied"
    whether or not anything had been. Plenty of terminals ignore it - macOS Terminal has no OSC 52
    at all, older VTE builds none either, tmux drops it unless told not to - and a promise that
    silently fails is worse than no promise. Nothing is lost: the code is there to read, the QR
    carries it, and a shift-drag selects it in any terminal.
    """
...
class _WidgetHooks(RunHooks):
    ...
    def __init__(self, screen: _RunScreen):
        ...
        self._notes = []
        # Set once the run directory exists (see ``log_into``). A resident run used to leave no
        # alma-certify.log at all, so the one front-end whose output you cannot reliably copy was
        # also the one that wrote nothing down - and "paste me the log" had no answer.
        self._log_path = None

    def log_into(self, run_dir) -> None:
        """Start appending to this run's own log file.

        Called once the run directory is known, which is after ``prepare_run``: everything logged
        before that point is the interface deciding what to run, not the run.
        """
        self._log_path = run_log_path(run_dir)
    ...
    def log(self, msg: str) -> None:
        # The file gets the timestamped line, exactly as a terminal run's does; the screen gets
        # the message alone. The run directory's log is an artifact other people read, and it has
        # to be the same artifact either way round; the screen is narrow and already says when
        # things happened by the order they appear in.
        write_log_line(self._log_path, msg)
...
class _SelectableLog(RichLog):
    """A ``RichLog`` whose output can be selected with the mouse, and therefore copied.

    Textual's text selection needs two things of a widget and ``RichLog`` provides neither.
    ``Widget.get_selection`` can only pull text out of a ``Text`` or ``Content`` visual, and a
    ``RichLog`` holds pre-rendered ``Strip``s, so it answers None and the selection comes back
    empty. And ``RichLog.render_line`` never calls ``Strip.apply_offsets``, which is what stamps
    each cell with the column it sits at - without those the screen cannot turn a drag into a
    span at all, which is why the run log could not even be *highlighted*, let alone copied.

    ``Log`` implements both and is still not a substitute: it takes plain strings and does not
    wrap, so every long line of a run - a dnf transaction, an lspci row - would run off the side
    instead of folding onto the next line.

    The highlight is applied to the whole line before it is cropped to the viewport, the way
    ``Log`` does it, so the span the screen computed is in the same coordinates as the line it
    lands on. That also keeps ``RichLog``'s own strip cache free of highlighted lines, since its
    key has no selection in it.
    """

    def get_selection(self, selection: TextSelection):
        """The text under ``selection``, as the reader sees it.

        One entry per *visual* line, because wrapping is on: a fold in a long line becomes a
        newline in what is copied. Joining the pre-wrap text instead would copy something other
        than the thing that was dragged over.
        """
    ...
    def render_line(self, y: int) -> Strip:
        ...
        if selection is None or index >= len(self.lines):
            strip = self._render_line(index, scroll_x, width)
        else:
            strip = self._highlight(self.lines[index], selection, index)
            strip = strip.crop_extend(scroll_x, scroll_x + width, self.rich_style)
        return strip.apply_style(self.rich_style).apply_offsets(scroll_x, index)

    def _highlight(self, strip: Strip, selection: TextSelection, index: int) -> Strip:
        """``strip`` with the selected part of line ``index`` painted, or unchanged."""
        span = selection.get_span(index)
        if span is None:
            return strip
        length = strip.cell_length
        characters = len(strip.text)
        start, end = span
        start = self._cell(strip, start, characters, length)
        end = length if end == -1 else self._cell(strip, end, characters, length)
        if start >= end:
            return strip
        before, selected, after = strip.divide([start, end, length])
        return Strip.join([before, selected.apply_style(self._selection_style()), after])

    def _selection_style(self) -> Style:
        """The selection's colours, keeping the log's own text colour where the theme allows.

        Not ``get_component_rich_style``, which is what this used to call and what made a
        selection unreadable: the default theme's ``screen-selection-foreground`` is
        ``transparent``, meaning "leave the text as it is", and flattening the component to a
        Rich style resolves that against its own background - so the style came back saying
        #094472 text on a #094472 background, a solid block where the words had been.

        Read off the component's styles instead, where transparent is still distinguishable by
        its alpha. A theme that names a real selection foreground - the ANSI ones say
        ``ansi_black`` - still gets it.
        """
        styles = self.screen.get_component_styles("screen--selection")
        # The default selection background is the primary colour at half alpha, so it tints the
        # line rather than replacing it. Taking ``rich_color`` straight off it drops the alpha
        # and paints a flat, heavier block; compositing it over the log's own background first
        # is what that half is for.
        _, background = self.background_colors
        style = Style(bgcolor=(background + styles.background).rich_color)
        if not styles.color.is_transparent:
            style += Style(color=styles.color.rich_color)
        return style

    @staticmethod
    def _cell(strip: Strip, index: int, characters: int, length: int) -> int:
        """The cell position of character ``index`` on this line, clamped to it.

        The span is in character offsets and the cuts are in cells, and the two part company the
        moment a run logs anything double-width - so the conversion is needed. It has to be
        clamped *before* the call rather than after, which is the bug this replaced: the span
        describes the drag, not this line, and ``index_to_cell_position`` raises
        ``NoCellPositionForIndex`` for an index the line does not reach instead of saturating.
        Dragging from column 60 down across a ten-character line asks for exactly that, and it
        came back from a live run as a crash.

        The zero guard is for a strip with no segments, which also raises. ``RichLog`` pads a
        blank line out to the render width so its own blanks are not that, but a zero-width one
        would be, and a guard that costs a comparison is cheaper than finding out.
        """
        if index <= 0:
            return 0
        if index >= characters:
            return length
        return strip.index_to_cell_position(index)
    ...
    **Leaving always works.** Two things conspired to make that untrue. Cancel is cooperative - it
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="run"):
            ...
            yield _SelectableLog(id="run-log", highlight=False, markup=False, wrap=True)
    ...
    def on_mount(self) -> None:
        self.query_one("#run-log", _SelectableLog).focus()
    ...
    def write_log(self, text: str) -> None:
        self.query_one("#run-log", _SelectableLog).write(text)
...
class _MainScreen(Screen):
    ...
    The buttons are a vertical stack, so up and down walk them. Tab does too, and always did, but
    tab is the convention for moving between *fields* on a form - a list of choices reads as
    something to arrow through, and the first thing anybody tries on it is an arrow key.
    """

    BINDINGS = [
        ("q", "quit_app", "Quit"),
        # Not shown in the footer: arrow keys on a list of buttons need no announcement, and the
        # footer is the place the reader looks for the keys they would not have guessed.
...
class _App(App):
    ...
    def build_run(self, argv):
        ...
        def run(hooks):
            ...
            # The run directory exists from here on, so the rest of the run is written down as
            # well as shown.
            hooks.log_into(state.run_dir)
