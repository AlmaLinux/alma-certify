...
Kept deliberately small and stdlib-only so both the base CLI and the vendored-Textual interface can
...
def run_log_path(run_dir: Optional[str]) -> Optional[str]:
    """Where a run's own log file lives, or None for a command that has no run directory."""
    return os.path.join(run_dir, "alma-certify.log") if run_dir else None


def write_log_line(log_path: Optional[str], msg: str) -> str:
    """Format one log line, append it to ``log_path`` when there is one, and return it.

    Shared by the terminal and the resident interface rather than written twice, so
    ``alma-certify.log`` means the same thing whichever front-end produced it: the same lines, in
    the same format, in the same place in the run directory. A reader comparing two runs, or a
    reviewer reading a bundle, should not have to know which one was used.
    """
...
class TerminalHooks(RunHooks):
    ...
    def log(self, msg: str) -> None:
        print(write_log_line(self._log_path, msg), flush=True)
