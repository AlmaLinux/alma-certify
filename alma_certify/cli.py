...
def _select_tui():
    ...
    ``(None, reason)``. Kept a lazy import so the base CLI never pulls Textual in on an ordinary,
    non-interface invocation.

    The import used to fail because the interface was a separate ``alma-certify-tui`` package
    somebody had not installed, and the message said so. It ships with the suite now, so a failure
    here is a broken installation or a checkout without the vendored tree - and telling either of
    those to install a package that no longer exists would send the reader somewhere useless.
    """
    try:
        import alma_certify_tui as frontend
    except Exception as exc:  # noqa: BLE001 - an unimportable interface is just unavailable
        return None, "the guided interface could not be loaded (%s)" % exc
...
def cmd_tui(args: argparse.Namespace) -> int:
    ...
    the interface) or, if the reader chooses, hands it off to run in plain output. Where the
    terminal cannot carry it - no TERM, a console ncurses has no description for - this prints the
    plain commands to use instead.
    """
