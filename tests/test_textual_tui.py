...
def test_the_auth_dialog_promises_no_clipboard():
    """``c`` used to write OSC 52 and then say "code copied" - a claim the terminal never
    acknowledges, and one that is false wherever OSC 52 is ignored (macOS Terminal, older VTE,
    tmux by default). The code is there to read, the QR carries it, and a shift-drag selects it."""
    @drive()
    async def go(pilot, app):
        ...
        await pilot.pause()
        go.keys = set(app.screen._bindings.key_to_bindings)
        go.text = " ".join(str(w.render()) for w in app.screen.query(Static))
        await pilot.press("c")
        await pilot.pause()
        go.clip = app.clipboard
        go.still_there = isinstance(app.screen, _AuthScreen)

    _run(go)
    assert "c" not in go.keys, "no key promises a copy the terminal may refuse"
    assert "copy" not in go.text.lower()
    assert go.clip == "", "pressing c writes nothing"
    assert go.still_there is True, "and does not take the dialog down either"
    assert "WXYZ-7788" in go.text, "the code is still readable, which is the point"
...
def test_arrow_keys_walk_the_opening_menu():
    """Tab always worked, but a list of choices reads as something to arrow through, and an arrow
    key is the first thing anybody tries on it."""
    @drive()
    async def go(pilot, app):
        await pilot.pause()
        go.first = app.focused.id
        await pilot.press("down")
        await pilot.pause()
        go.second = app.focused.id
        await pilot.press("down")
        await pilot.pause()
        go.third = app.focused.id
        await pilot.press("up")
        await pilot.pause()
        go.back = app.focused.id
        await pilot.press("q")
        await pilot.pause()

    _run(go)
    assert go.first == "menu-validate", "the menu opens with the first choice focused"
    assert go.second == "menu-benchmark"
    assert go.third == "menu-run"
    assert go.back == "menu-benchmark", "and up walks back"


def test_an_arrowed_choice_is_the_one_that_opens():
    """Moving the focus has to move what Enter acts on, or the arrows are decoration."""
    @drive()
    async def go(pilot, app):
        await pilot.pause()
        await pilot.press("down")
        ...
        await pilot.press("enter")
        await pilot.pause()
        go.plan_what = getattr(app.screen, "plan", None) and app.screen.plan.what
        await pilot.press("escape")
        await pilot.pause()
        await pilot.press("q")
        await pilot.pause()
    ...
    _run(go)
    assert go.plan_what == "benchmark", "down then Enter opens the second choice, not the first"


def test_the_arrow_keys_stay_out_of_the_footer():
    """The footer is for keys somebody would not have guessed. Arrowing a list is not one."""
    @drive()
    async def go(pilot, app):
        await pilot.pause()
        go.shown = {b.key for b in app.screen._bindings.shown_keys}
        await pilot.press("q")
        await pilot.pause()

    _run(go)
    assert "q" in go.shown
    assert not {"up", "down"} & go.shown


# --- copying the output ----------------------------------------------------------
#
# "I'm still not able to copy from the output box in the TUI" / "I can't even highlight". The
# output box is a RichLog, which renders pre-built Strips: Textual's selection could neither
# extract text from it (``Widget.get_selection`` only reads a Text/Content visual) nor work out
# which column the mouse was over (``RichLog.render_line`` never applied Strip offsets). So a
# drag did nothing visible and the copy action came back empty.


def _started_run(pilot, app, lines):
    """Open the run screen on a finished run that logged ``lines``."""
    def run(hooks):
        for line in lines:
            hooks.log(line)
        return 0
...
def _select(screen, log, start, end):
    """Select from ``start`` to ``end``, as a drag across the log would."""
    from textual.geometry import Offset
    from textual.selection import Selection

    screen.selections = {log: Selection(Offset(*start), Offset(*end))}
    return screen.get_selected_text()


def test_the_output_box_hands_back_the_text_under_a_selection():
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["first line", "second line", "third line"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        go.selected = _select(app.screen, log, (6, 0), (6, 1))
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.selected == "line\nsecond"


def test_a_selection_of_everything_is_the_whole_log():
    """``Selection(None, None)`` is what a select-all produces, and it used to produce nothing."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["alpha", "beta"])
        from textual.selection import Selection

        log = app.screen.query_one("#run-log", _SelectableLog)
        app.screen.selections = {log: Selection(None, None)}
        go.selected = app.screen.get_selected_text()
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.selected == "alpha\nbeta"


def test_every_rendered_cell_knows_which_column_it_is():
    """What makes a drag into a selection at all. Without the offsets the screen has no way to
    turn a mouse position into a span, which is why the log could not even be highlighted."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["hello"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        strip = log.render_line(0)
        go.offsets = [
            segment.style.meta.get("offset") for segment in strip if segment.style is not None
        ]
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.offsets and all(offset is not None for offset in go.offsets), go.offsets


def test_the_selected_part_of_a_line_is_painted():
    """A selection you cannot see is not one. The style has to land on the selected span and
    nowhere else, so the reader can tell what they are about to copy."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["abcdefghij"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        go.plain = log.render_line(0).render(app.console)
        _select(app.screen, log, (2, 0), (5, 0))
        go.painted = log.render_line(0).render(app.console)
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.painted != go.plain, "the selected span renders no differently from the rest"


def test_an_unselected_log_renders_exactly_as_it_did():
    """The highlight path must not change what an ordinary run looks like."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["ordinary output"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        go.text = log.render_line(0).text
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.text.startswith("ordinary output")


# --- the run's own log file ------------------------------------------------------
#
# The resident interface wrote nothing to disk: alma-certify.log existed only for terminal runs.
# So the one front-end whose output is hard to copy was also the one that kept no copy, and
# "send me the log" had no answer for anybody who used it.


def test_a_resident_run_writes_its_log_file(tmp_path):
    def run(hooks):
        hooks.log_into(str(tmp_path))
        hooks.log("collecting inventory")
        hooks.log("running validate.cpu.functional")
        return 0
    ...
    _run(go)
    written = (tmp_path / "alma-certify.log").read_text().splitlines()
    assert [line.split("] ", 1)[1] for line in written] == [
        "collecting inventory", "running validate.cpu.functional",
    ]


def test_the_file_is_the_one_a_terminal_run_would_have_written(tmp_path):
    """Same name, same place, same format. A bundle's log should not tell a reviewer which
    front-end produced it."""
    from alma_certify.runhooks import TerminalHooks

    terminal = tmp_path / "terminal"
    resident = tmp_path / "resident"
    terminal.mkdir()
    resident.mkdir()
    TerminalHooks(str(terminal)).log("a line")

    def run(hooks):
        hooks.log_into(str(resident))
        hooks.log("a line")
        return 0
    ...
    _run(go)
    from_terminal = (terminal / "alma-certify.log").read_text()
    from_resident = (resident / "alma-certify.log").read_text()
    # Timestamps differ; everything else about the line must not.
    assert from_terminal.split("] ", 1)[1] == from_resident.split("] ", 1)[1]
    assert from_terminal.startswith("[") and from_resident.startswith("[")


def test_nothing_is_written_before_there_is_a_run_to_write_about(tmp_path):
    """Logging starts at the run directory, not at the interface. What the operator did in the
    form is not part of the run's record, and there is nowhere to put it anyway."""
    def run(hooks):
        hooks.log("before the run directory exists")
        hooks.log_into(str(tmp_path))
        hooks.log("after")
        return 0
    ...
    async def go(pilot, app):
        ...
        await pilot.click("#start")
        await _wait_for_code(pilot, app)
        go.shown = _log_text(app.screen)
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    written = (tmp_path / "alma-certify.log").read_text()
    assert "before the run directory exists" not in written
    assert "after" in written
    assert "before the run directory exists" in go.shown, "the screen still shows it"


def test_the_run_starts_logging_as_soon_as_it_has_somewhere_to_log(monkeypatch, tmp_path):
    """The wiring, pinned. ``log_into`` is called with the prepared run's directory - miss it and
    every resident run silently writes nothing, which is exactly the state this fixes."""
    from alma_certify import cli

    class _State:
        run_dir = str(tmp_path)

    monkeypatch.setattr(cli, "prepare_run", lambda argv, hooks: (_State(), object(), ["validate"]))
    monkeypatch.setattr(cli, "execute", lambda state, config, run_types, hooks, debug: 0)

    class _Hooks:
        def __init__(self):
            self.told = None

        def log_into(self, run_dir):
            self.told = run_dir

    hooks = _Hooks()
    _app().build_run(["validate"])(hooks)

    assert hooks.told == str(tmp_path)


def test_a_selection_lands_on_the_right_characters_when_they_are_wide():
    """The span arrives as character offsets and the strip is cut at cell positions, and the two
    part company the moment a run logs anything double-width - a device name in Japanese, an
    emoji in a vendor string. Selecting the second and third characters must paint those two."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["日本語abc"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        _select(app.screen, log, (1, 0), (3, 0))
        go.segments = [segment.text for segment in log.render_line(0)]
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.segments[:2] == ["日", "本語"], go.segments


def test_selecting_everything_paints_every_line_to_its_end():
    """``get_span`` answers -1 for "to the end of this line", which is what a select-all produces
    on every line but the last. Treated as a column, it is a cut before the start and the line
    comes out unpainted - selected text that does not look selected."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["alpha", "beta"])
        from textual.selection import Selection
    ...
    _run(go)
    assert go.painted != go.plain, "a select-all leaves the lines looking unselected"


def test_dragging_past_the_end_of_a_short_line_does_not_crash():
    """Reported from a live run: NoCellPositionForIndex out of index_to_cell_position while
    highlighting. A drag that starts at column 60 and crosses a ten-character line asks that line
    for a character it does not have, and the conversion raises rather than saturating - so the
    clamp has to happen before the call, not after it."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["a very long line of run output indeed", "short", "also"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        _select(app.screen, log, (30, 0), (30, 2))
        go.rendered = [log.render_line(y).text for y in range(3)]
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.rendered[1].startswith("short"), "the short line still renders"


def test_a_selection_spanning_a_blank_line_keeps_it():
    """A run log is full of blank lines - every ``out()`` separating a block writes one - and a
    selection dragged over them has to come back with them in it, or pasted output loses the
    shape that made it readable.

    Not a regression test for the crash above, though it was written as one: ``RichLog`` pads a
    blank line out to the render width, so it has a segment and converts fine. What it does pin
    is the extraction across one.
    """
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["before", "", "after"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        _select(app.screen, log, (2, 0), (3, 2))
        go.rendered = [log.render_line(y).text for y in range(3)]
        go.selected = app.screen.get_selected_text()
        await pilot.press("ctrl+q")
        await pilot.pause()
    ...
    _run(go)
    assert go.rendered[0].startswith("before")
    assert go.selected == "fore\n\naft"


def test_a_selection_reaching_past_the_last_character_paints_to_the_end():
    """Clamping must not quietly stop painting. Dragging off the right-hand end of a line selects
    all of it, and it has to look selected."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["abcdef"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        go.plain = log.render_line(0).render(app.console)
        _select(app.screen, log, (0, 0), (99, 0))
        go.painted = log.render_line(0).render(app.console)
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.painted != go.plain


def test_highlighted_text_is_still_readable():
    """Reported after the first cut: the highlight "completely blocks out the actual text". The
    default theme's ``screen-selection-foreground`` is ``transparent`` - leave the text as it is -
    and flattening the component to a Rich style resolved that against its own background, so the
    style said #094472 text on a #094472 background. A solid block where the words had been."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["abcdefghij"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        go.unselected = next(iter(log.render_line(0))).style.color
        _select(app.screen, log, (2, 0), (6, 0))
        selected = [seg for seg in log.render_line(0) if seg.text == "cdef"]
        go.pair = [(seg.style.color, seg.style.bgcolor) for seg in selected]
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.pair, "the selected span did not come back as its own segment"
    for color, bgcolor in go.pair:
        assert color != bgcolor, "selected text is painted in its own background colour"
        assert color == go.unselected, "selecting text must not change what colour it is"


def test_the_highlight_tints_rather_than_replaces():
    """The selection background is the primary colour at half alpha so it tints the line. Taking
    the colour straight off the component drops that alpha and paints a flat, heavier block."""
    @drive()
    async def go(pilot, app):
        await _started_run(pilot, app, ["abcdefghij"])
        log = app.screen.query_one("#run-log", _SelectableLog)
        styles = app.screen.get_component_styles("screen--selection")
        go.declared = styles.background
        _select(app.screen, log, (2, 0), (6, 0))
        go.painted = [seg.style.bgcolor for seg in log.render_line(0) if seg.text == "cdef"]
        await pilot.press("ctrl+q")
        await pilot.pause()

    _run(go)
    assert go.declared.a < 1, "the theme stopped asking for a tint; this test is now moot"
    assert go.painted and go.painted[0] != go.declared.rich_color, (
        "the half-alpha tint was painted as a solid colour"
    )
